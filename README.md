...
Having those parameters, the notebook allows to compute the positions $x$ and$y$, the velocity $\dot{x}$ and$\dot{y}$, and the acceleration $\ddot{x}$ and$\ddot{y}$ of each masses. Those results are saved in a csv file. 
