...
Having those parameters, the notebook allows to compute the positions $x$ and$y$, the velocity $\dot{x},\dot{y}$and acceleration of each masses in $\hat{x}$ and $\hat{y}$ direction. Those results are saved in a csv file. 
